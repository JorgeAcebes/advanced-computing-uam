...
- **Verlet Algorithm**: Integration of Newton’s equations of motion for particle trajectory calculations.
- **Lennard-Jones Potential**: Modeling interatomic interactions using periodic boundary conditions and analyzing solid-liquid phase transitions.

### 9. Potentials & Elliptic PDEs (Boundary Value Problems)
- **Finite Differences**: Solving the Laplace ($\nabla^2 \phi = 0$) and Poisson equations using Jacobi, Gauss-Seidel, and Successive Over-Relaxation (SOR) methods.
- **Magnetostatics**: Numerical computation of the magnetic field $\mathbf{B}$ generated by steady currents and solenoids.

### 10. Diffusion & Parabolic PDEs (Initial Value Problems)
...
### 11. Waves & Hyperbolic PDEs (Initial Value Problems)
