...
Para sortear esta limitación computacional, se diseñó e implementó un esquema de extrapolación lineal. El procedimiento consiste en amplificar artificialmente la influencia gravitacional de Júpiter mediante un factor multiplicativo  (en este caso, abarcando el intervalo desde $5$ hasta $11.5$), trasladando la dinámica hacia un régimen perturbativo donde el efecto de arrastre es más pronunciado y estable. Para cada configuración másica iterada, se resuelve el sistema acoplado utilizando el integrador simpléctico de \textit{Velocity Verlet}. Seguidamente, se tabula el ángulo del afelio en cada tránsito y se extrae la tasa de precesión $\dot{w}$ mediante una primera regresión lineal. 
...
multiplicative = np.linspace(5, 11.5,100)
w = []
for factor_m_J in multiplicative:
    ...
    # Guardamos resultados para poder tenerlos disponibles a demanda
    np.savetxt('halley_and_jupyter.txt', data_matrix, comments='')
    np.savetxt('precesion.txt', data_precesion, comments='')

    #  Desempaquetamos los resultados 
    x_h, y_h, x_J, y_J, t = np.loadtxt('halley_and_jupyter.txt', unpack=True)    
...
regress = linregress(multiplicative, np.array(w))
pend = regress.slope
C = regress.intercept
